...
for (const width of [1440, 390]) {
  test(`Compose authoring reviews text-file rows and rejects duplicate paths at ${width}px`, async ({ page }) => {
    ...
    await dialog.getByTestId('container-add-file').click()
    await dialog.locator('[name="file-path-1"]').fill('site/logo.png')
    ...
    await dialog.locator('input[type="file"]').nth(1).setInputFiles({ name: 'logo.png', mimeType: 'image/png', buffer: picture })
    await expect(dialog.locator('[name="file-content-1"]')).toHaveCount(0)
    await dialog.getByRole('button', { name: 'Preview files', exact: true }).click()
    ...
    const assetPreview = dialog.getByTestId('container-file-preview').filter({ hasText: '/site/logo.png' })
    await expect(assetPreview).toContainText(`${picture.length} bytes`)
    await expect(assetPreview.getByTestId('asset-download')).toHaveAttribute('href', `data:application/octet-stream;base64,${picture.toString('base64')}`)

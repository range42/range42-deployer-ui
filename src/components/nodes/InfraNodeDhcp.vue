...
<template>
  ...
  >
    <div class="flex items-center justify-between mb-3">
      ...
      <div class="text-xs opacity-70 uppercase tracking-wide">DHCP</div>
    </div>
    ...
    <div class="space-y-2">
      ...
      <div class="text-xs opacity-80 space-y-1">
        <div v-if="data.config?.scope">
          Scope: {{ data.config.scope }}
        </div>
        <div v-if="data.config?.gateway">
          Gateway: {{ data.config.gateway }}
        </div>
      </div>
    </div>

    <Handle type="target" :position="Position.Left" class="!bg-cyan-500 !border-cyan-600" />
    <Handle type="source" :position="Position.Right" class="!bg-cyan-500 !border-cyan-600" />
  </div>
</template>
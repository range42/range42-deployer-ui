          ...
          <template v-if="!preview">
            ...
            <fieldset v-for="vm in draft.vms" :key="vm.node_id" class="border border-base-300 rounded-lg p-3 mb-3 min-w-0">
              ...
              <div class="grid gap-3 sm:grid-cols-3">
                ...
                <label class="form-control gap-1"><span>Guest SSH user</span><input v-model="vm.ssh_user" class="input input-bordered w-full" data-testid="scenario-vm-ssh-user" maxlength="32" /></label>
                <label class="form-control gap-1"><span>Cloud-init DNS servers</span><input v-model="vm.dns_servers" class="input input-bordered w-full" data-testid="scenario-vm-dns" maxlength="128" placeholder="Inherit template DNS" /><span class="text-xs text-base-content/70">Up to three IPv4 addresses, separated by spaces or commas. Blank retains template DNS.</span></label>
                <label class="form-control gap-1"><span>Cloud-init search domain</span><input v-model="vm.dns_search_domain" class="input input-bordered w-full" data-testid="scenario-vm-domain" maxlength="253" placeholder="Inherit template domain" /></label>
                <label class="form-control gap-1"><span>Clone destination storage</span><input v-model="vm.storage" class="input input-bordered w-full" data-testid="scenario-vm-storage" placeholder="Inherit template storage" maxlength="64" /><span class="text-xs text-base-content/70">Choose a VM image pool on the deployment host. Blank inherits template storage. This applies to new clones only.</span></label>

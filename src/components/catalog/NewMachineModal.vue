...
const { t } = useI18n({ useScope: 'local', messages: {
  en: { title: 'New VM blueprint', intro: 'Define a reusable machine and its resource requirements.', category: 'Category', target: 'Target',
    ...
    portable: 'Leave template and storage blank for a portable public blueprint. Choose a template on the target host when configuring a project. Clones inherit its storage; a storage preference does not move disks.',
  ...
  fr: { title: 'Nouveau modèle de machine virtuelle', intro: 'Définissez une machine réutilisable et ses ressources.', category: 'Catégorie', target: 'Cible',
    ...
    portable: 'Laissez le template et le stockage vides pour un modèle public portable. Choisissez un template sur l’hôte cible. Les clones héritent de son stockage ; la préférence ne déplace pas les disques.',
  ...
  jp: { title: 'VM 定義を作成', intro: '再利用できるマシンと必要なリソースを定義します。', category: 'カテゴリ', target: '対象',
    ...
    portable: '公開用の定義ではテンプレートとストレージを空欄にし、設定時に対象ホストのテンプレートを選択します。クローンはそのストレージを継承し、指定した設定でディスクは移動しません。',

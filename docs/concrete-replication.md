...
- Stable source NIC keys are sent as `nic_key`; a response must match both current index and key. Draft changes invalidate review. Authoring leases cover VM IDs and NIC addresses; preview or reservation alone creates no durable deployment ownership. The deployment form can transfer a matching lease into a backend-owned [durable claim](deployment-allocations.md). Subnet/VNet ownership is still not reserved by this flow.
...
Real GitHub and GitLab acceptance each passed 13 assertions against UI `45a4f04`: pinned reopening, unchanged source branches and mains, a new working branch from the reviewed revision, identical public-review/private-direct snapshots, original source forms and roster, three literal VMs/two networks, exclusions, binary files, and omission of private values. Reports: [GitHub](acceptance/replication-github-20260911.json), [GitLab](acceptance/replication-gitlab-20260911.json). The reusable local harness is `/tmp/r42-replication-provider-acceptance/`; it records the reviewed source and compiled kit hash. No replica guests were deployed by these provider tests.

At that historical acceptance checkpoint, the shared installation ran API `052e089` with UI `45a4f04`. A fresh HTTP browser reopened the real public GitHub revision, then reserved all three literal VMs through the shared backend, applied the mapping, retained source authoring on reload, and released the lease (HTTP 200 for the lease read and 204 for release). The temporary lease was confirmed removed and the original set of 47 guests remained unchanged; no guests or SDN resources were created. The evidence is in the [shared browser report](acceptance/replication-shared-browser-20260911.json). These results do not establish successful replicated guest provisioning.

Automatic allocation originally used the backend `nic_key` extension in paired API `052e089` (PR138). Incomplete network declarations show a preparation error before the reservation panel is offered. The assessed UI `7c0aeda` / API `553af0a` pair additionally supports lease consumption into durable deployment claims and explicit reviewed release; see [allocation controls](deployment-allocations.md). Subnet/VNet reservation, topology growth/resume and destructive shrink remain separate work. Later shared full attempts failed during cloud-init; successful diagnostics, cleanup and claim release must not be reported as full three-guest acceptance. See the [current baseline and evidence map](ui-backend-capability-matrix.md).
